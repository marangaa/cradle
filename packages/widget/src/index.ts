...
class CradleCharacter extends HTMLElement {
  ...
  private render() {
    this.shadow.innerHTML = [
      ...
      '.panel{position:absolute;bottom:100%;right:6px;margin-bottom:16px;width:min(320px,calc(100vw - 32px));padding:18px 20px 16px;background:linear-gradient(145deg,rgba(20,20,28,0.96) 0%,rgba(10,10,14,0.95) 100%);backdrop-filter:blur(20px);border:1.5px solid rgba(168,85,247,0.25);border-radius:20px 20px 4px 20px;box-shadow:0 24px 50px -12px rgba(0,0,0,0.75),0 0 24px rgba(168,85,247,0.12);transform-origin:bottom right;transition:opacity .2s ease,transform .2s ease}.panel::after{content:"";position:absolute;bottom:-10px;right:28px;width:18px;height:18px;background:rgba(10,10,14,0.95);border-right:1.5px solid rgba(168,85,247,0.25);border-bottom:1.5px solid rgba(168,85,247,0.25);transform:rotate(45deg)}.panel[hidden]{display:none}',
      '.name-tag{position:absolute;top:-13px;left:18px;display:inline-flex;align-items:center;gap:5px;padding:3px 12px;background:linear-gradient(135deg,#6366f1 0%,#a855f7 50%,#ec4899 100%);border-radius:999px;box-shadow:0 4px 14px rgba(168,85,247,0.45)}.title{color:#ffffff;font-size:.68rem;font-weight:850;letter-spacing:.05em;text-transform:uppercase;font-family:ui-sans-serif,system-ui,sans-serif;line-height:1}.greeting{margin:2px 0 0;color:#f1f5f9;font-size:.86rem;line-height:1.55;font-weight:500}',
      ...
      '</style>',
      '<div class="shell"><section class="panel" hidden aria-label="Website character"><div class="name-tag"><span class="title">Loading...</span></div><p class="greeting">Connecting to companion...</p></section><button class="trigger" type="button" aria-label="Open website character" aria-expanded="false"><span class="companion" aria-hidden="true"></span></button></div>',
