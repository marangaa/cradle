...
class CradleResident extends HTMLElement {
  ...
  connectedCallback() {
    ...
    form.addEventListener("submit", async (event) => {
      ...
      const response = await fetch(`${apiBase}/api/chat`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ installationId, visitorId, conversationId: crypto.randomUUID(), message }) });
