...
class CradleCharacter extends HTMLElement {
  ...
  private render() {
    this.shadow.innerHTML = [
      ...
      '.panel{position:absolute;bottom:100%;right:8px;margin-bottom:14px;width:min(320px,calc(100vw - 32px));padding:16px 18px;background:rgba(15,15,18,0.94);backdrop-filter:blur(16px);border:1.5px solid rgba(255,255,255,0.15);border-radius:18px;box-shadow:0 20px 45px -10px rgba(0,0,0,0.65),0 0 20px rgba(165,180,252,0.08);animation:float 4s ease-in-out infinite;transform-origin:bottom right}.panel::after{content:"";position:absolute;bottom:-9px;right:32px;width:16px;height:16px;background:rgba(15,15,18,0.94);border-right:1.5px solid rgba(255,255,255,0.15);border-bottom:1.5px solid rgba(255,255,255,0.15);transform:rotate(45deg)}.panel[hidden]{display:none}',
      '.header{display:flex;align-items:center;justify-between;margin-bottom:6px}.title{display:block;color:#f8fafc;font-size:.94rem;font-weight:780;letter-spacing:-.025em}.badge{display:inline-flex;align-items:center;gap:5px;padding:2px 7px;background:rgba(34,197,94,0.12);border:1px solid rgba(34,197,94,0.25);border-radius:999px;color:#4ade80;font-family:ui-monospace,monospace;font-size:.62rem;font-weight:700;letter-spacing:.04em;text-transform:uppercase}.badge-dot{width:5px;height:5px;border-radius:50%;background:#22c55e;box-shadow:0 0 6px #22c55e}.greeting{margin:0;color:#cbd5e1;font-size:.82rem;line-height:1.52}',
      '.trigger{display:grid;width:94px;height:102px;place-items:center;border:0;background:transparent;box-shadow:none;cursor:grab;touch-action:none}.trigger:active{cursor:grabbing}.trigger:focus-visible{outline:3px solid #a5b4fc;outline-offset:3px}.trigger .companion{width:88px;height:96px;background-repeat:no-repeat;background-size:800% 900%}@media (prefers-reduced-motion:reduce){.panel{animation:none!important}.companion{animation:none!important}}',
      '</style>',
      '<div class="shell"><section class="panel" hidden aria-label="Website character"><div class="header"><strong class="title">Loading</strong><span class="badge"><span class="badge-dot"></span>Online</span></div><p class="greeting"></p></section><button class="trigger" type="button" aria-label="Open website character" aria-expanded="false"><span class="companion" aria-hidden="true"></span></button></div>',
