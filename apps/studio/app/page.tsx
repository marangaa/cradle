  ...
  return <main className="studio">
    ...
    {!result ? <><form className="discovery" onSubmit={discover}><label htmlFor="site">Start with a public website</label><div><input id="site" type="url" required value={url} onChange={(event) => setUrl(event.target.value)} placeholder="https://yourcompany.com" /><button disabled={busy}>{busy ? "Reading the site…" : "Discover the shape"}</button></div><small>Public, same-domain crawl. You review the source material before generation.</small></form><form className="resume" onSubmit={resume}><label htmlFor="installation">Already started?</label><input id="installation" required value={resumeInstallationId} onChange={(event) => setResumeInstallationId(event.target.value)} placeholder="Installation ID" /><input required value={resumeManagementKey} onChange={(event) => setResumeManagementKey(event.target.value)} placeholder="Owner credential" /><button disabled={busy}>Resume installation</button></form></> : <>
      ...
      {revision?.identity && <>
        ...
        {revision.status === "selected" && <section className="published"><p className="kicker">03 / Asset review</p><h2>{published ? "Published." : revision.error ? "Generation needs another try." : packReady ? "Your state pack is ready." : `Preparing assets (${draftStates.size}/7).`}</h2><p>{published ? "The widget can now fetch the immutable canonical asset." : revision.error ? revision.error : packReady ? "Review every generated state, then publish the complete pack." : "Cradle is deriving each interaction state from one canonical base."}</p><AssetPackPreview assets={assets} previewUrls={previewUrls} />{revision.error && <button onClick={() => selectDirection(revision.selectedDirectionId!)} disabled={busy}>Retry asset generation</button>}{!published && !revision.error && <button onClick={publishAssets} disabled={busy || !packReady}>Publish asset pack</button>}{published && <InstallSnippet installationId={result.installation.id} />}</section>}
      </>}
    </>}
